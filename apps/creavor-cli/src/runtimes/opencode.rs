...
pub fn run() -> anyhow::Result<()> {
    ...
    if let Some(ref url) = original_url {
        if !is_broker_address(url, settings.broker.port) {
            settings.set_upstream("opencode", url);
            settings.save()?;
        }
    }

    let session_id = session::generate_session_id("opencode");
    let broker_healthy = broker::health_check().is_ok();

    let binary = find_binary("opencode")?;

    if !broker_healthy {
        tracing::warn!("broker not detected — launching opencode without proxy");
    ...
    tracing::info!(proxy_url = %proxy_url, session_id = %session_id, "launching opencode with proxy");
...
pub fn config() -> anyhow::Result<()> {
    ...
    if let Some(ref url) = original_url {
        if is_broker_address(url, settings.broker.port) {
            println!("opencode is already configured to use broker");
            return Ok(());
        }
        settings.set_upstream("opencode", url);
        settings.save()?;
    ...
    RuntimeType::OpenCode.write_api_url(&proxy_url)?;
    println!("opencode configured to use broker at {}", proxy_url);
    Ok(())
}

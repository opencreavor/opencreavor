...
pub fn run() -> anyhow::Result<()> {
    ...
    let original_url = RuntimeType::Codex.read_current_api_url();

    if let Some(ref url) = original_url {
        if !is_broker_address(url, settings.broker.port) {
            settings.set_upstream("codex", url);
            settings.save()?;
        }
    }

    let session_id = session::generate_session_id("codex");
    let broker_healthy = broker::health_check().is_ok();

    let binary = find_binary("codex")?;

    if !broker_healthy {
        tracing::warn!("broker not detected — launching codex without proxy");
    ...
    tracing::info!(proxy_url = %proxy_url, session_id = %session_id, "launching codex with proxy");
...
pub fn config() -> anyhow::Result<()> {
    ...
    if let Some(ref url) = original_url {
        if is_broker_address(url, settings.broker.port) {
            println!("codex is already configured to use broker");
            return Ok(());
        }
        settings.set_upstream("codex", url);
        settings.save()?;
    ...
    RuntimeType::Codex.write_api_url(&proxy_url)?;
    println!("codex configured to use broker at {}", proxy_url);
    Ok(())
}
